...
def collect_device_health_self_assessment(device):
    ...
    yield Metric("device_smart_healthy", device.base_labels, self_assessment_passed)
    ...
    for entry in reader:
        ...
        if entry["name"] in smart_attributes_whitelist and entry["name"] not in seen:
            ...
            for col in "value", "worst", "threshold", "raw_value":
                yield Metric("attr_{col}".format(col=col), labels, entry[col])
...
def collect_disks_smart_metrics(wakeup_disks):
    ...
    for device in find_devices():
        ...
        yield Metric("device_smart_available", device.base_labels, smart_available)
        yield Metric("device_smart_enabled", device.base_labels, smart_enabled)
...
def get_metrics(wakeup_disks=True):
    ret = []
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "-s", "--wakeup-disks", dest="wakeup_disks", action="store_true"
    )
